...
#ifdef USE_FSMC
extern volatile uint16_t * LCD_REG;
...
using SetPanel_D = Register1Write<REG::SET_PANEL_B>;
...
using SetVCOM_D =Register1Write<REG::SET_VCOM_D>;
...
using SetSTBA =RegisterWrite<REG::SET_STBA, 6>;
...
using SetCYC = RegisterWrite<REG::SET_DISPLAY_MODE, 7>;
using SetTearLine = RegisterWrite<REG::TEARLINE, 2>;
...
public:
    ...
    void drawPixelInternal(uint16_t x, uint16_t y, Color16Bit color) override;
    void setDriverQuality(DriverQuality driverQuality);
    void drawFastSample(uint16_t x, uint16_t value);
protected:
